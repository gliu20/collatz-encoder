...
def read_file(file, encoder=pass_through, is_verbose=False):
    ...
    if is_verbose:
        print(
            f'[Info] Read file in {(time.process_time_ns() - start) / 1e6} ms')
    ...
    if is_verbose:
        print(
            f'[Info] Decoded file in {(time.process_time_ns() - start) / 1e6} ms')
...
def write_file(file, num, encoder=pass_through, is_verbose=False):
    ...
    if is_verbose:
        print(
            f'[Info] Encoded file in {(time.process_time_ns() - start) / 1e6} ms')
    ...
    if is_verbose:
        print(
            f'[Info] Wrote file in {(time.process_time_ns() - start) / 1e6} ms')
